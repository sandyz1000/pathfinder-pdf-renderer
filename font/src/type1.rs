...
use crate::postscript::{RefItem, Vm};
use crate::{
    v, Context, Font, FontError, Glyph, GlyphId, HMetrics, IResultExt, Info, Name, State, TryIndex,
    Value, R,
};
...
use nom::{
    number::complete::{be_i32, be_u8, le_u32, le_u8},
    IResult,
};
...
impl Type1Font {
    ...
    pub fn from_vm(vm: Vm) -> Result<Self, FontError> {
        ...
        debug!(
            "FontDict keys: {:?}",
            font_dict.iter().map(|(k, _)| k).format(", ")
        );
        debug!(
            "Private keys: {:?}",
            private_dict.iter().map(|(k, _)| k).format(", ")
        );
        ...
        let weight = font_info
            .and_then(|d| d.get_str("Weight"))
            .and_then(|s| match s {
                "Light" => Some(300),
                "Regular" => Some(400),
                "Medium" => Some(500),
        ...
        for (name, item) in char_strings.string_entries() {
            ...
            let (index, _) = glyphs.insert_full(
                name.to_owned(),
                Glyph {
                    metrics: HMetrics {
        ...
        for item in encoding.iter() {
            match item {
                ...
                RefItem::Literal(name) => match std::str::from_utf8(name) {
                    Ok(name) => {
        ...
        let bbox = font_dict
            ...
            .map(|val| {
                ...
                require!(arr.len() == 4);
                let (a, b, c, d) = expect!(
                    TupleElements::from_iter(arr.iter().filter_map(|v| v.as_f32())),
                    "needs 4 numbers"
                );
        ...
        let (a, b, c, d, e, f) =
            TupleElements::from_iter(font_matrix.iter().map(|item| item.as_f32().unwrap()))
                .unwrap();
...
fn parse_binary<'a>(vm: &mut Vm, data: &'a [u8]) {
    ...
    vm.parse_and_exec(&decoded).unwrap();
}
                    ...
                    16 => {
                        ...
                        match subr_nr {
                            ...
                            0 => {
                                ...
                                let (_ref, c0, c1, p2, c3, c4, p5) = expect!(
                                    TupleElements::from_iter(flex_sequence.into_iter()),
                                    "can't parse flex sequence"
                                );
                                //require_eq!(p5, v(x, y));

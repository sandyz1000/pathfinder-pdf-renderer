...
use winit::event::{
    ElementState as WinitElementState, Event, MouseButton, MouseScrollDelta, StartCause,
    WindowEvent,
};
                ...
                WindowEvent::MouseWheel { delta, .. } => {
                    let delta = match delta {
                        MouseScrollDelta::PixelDelta(PhysicalPosition { x: dx, y: dy }) => {
                            Vector2F::new(dx as f32, dy as f32) * ctx.pixel_scroll_factor
                        }
                        MouseScrollDelta::LineDelta(dx, dy) => {
                            Vector2F::new(dx as f32, dy as f32) * ctx.line_scroll_factor
                        }
                    };
